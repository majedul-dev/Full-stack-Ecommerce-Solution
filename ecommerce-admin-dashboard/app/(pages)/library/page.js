...
const AssetsLibrary = () => {
  ...
  const fetchAssets = async (cursor) => {
    try {
      const res = await fetch(`${process.env.BACKEND_URL}/api/assets?cursor=${cursor || ''}&search=${searchQuery}`);
  ...
  const deleteAssets = async () => {
    ...
    try {
      const response = await axios.delete(`${process.env.BACKEND_URL}/api/assets`, {
